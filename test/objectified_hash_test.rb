...
class ObjectifiedHashTest < Minitest::Spec
  before do
    @hash = { a: 1, "string" => "string", symbol: :symbol, items: [{ a: 1 }] }
    @oh = Hashash::ObjectifiedHash.new(@hash)
  end

  it "should objectify hash" do
    @oh.a.must_equal @hash[:a]
    @oh.string.must_equal @hash["string"]
    @oh.symbol.must_equal @hash[:symbol]
    @oh.items.length.must_equal 1
    @oh.items[0].to_hash.must_equal @hash[:items][0]
